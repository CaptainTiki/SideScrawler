...
func generate() -> void:
	...
	while placed.size() - 1 < target_room_count and _frontier.size() > 0 and fails < 50:
		print("Frontier size=", _frontier.size(), " placed rooms=", placed.size())
		var a_conn = _frontier.pop_back()
		# find a candidate room with a compatible connector
		var candidate_scene := _pick_room_from_catalog()
		...
		# remove the used connector from B, and add B's other connectors to frontier
		var new_conns := _collect_connectors(b_room)
		# filter out the one we used (compare by instance id)
		new_conns = new_conns.filter(func(c): return c != b_conn)
		_frontier.append_array(new_conns)

		# also remove 'a_conn' from further consideration (it’s now used)
		_frontier = _frontier.filter(func(c): return c != a_conn)
		
		print("Frontier size=", _frontier.size(), " placed rooms=", _placed_rooms.size())
